...
public class ShotGun : Gun
{
    ...
    protected override void Fire()
    {
        ...
        for (int i = 0; i < bulletNum; i++)
        {
            ...
            if (bulletNum % 2 == 1)
            {
                BulletFactory.Instance.CreatBullet("shotGun", muzzlePos.position,
                Quaternion.AngleAxis(bulletAngel * (i - median), Vector3.forward) * direction, color, damage, direction, speed);
                // bullet.GetComponent<Bullet>().SetSpeed(Quaternion.AngleAxis(bulletAngel * (i - median), Vector3.forward) * direction, speed);
            }
            else
            {
                BulletFactory.Instance.CreatBullet("shotGun", muzzlePos.position, 
                Quaternion.AngleAxis(bulletAngel * (i - median) + bulletAngel / 2, Vector3.forward) * direction, color, damage, direction, speed);
