...
public class State_2 : YaSuoState
{
    private YaSuo Boss;
    ...
    {
        Boss.dist = Vector3.Distance(Boss.player.position, Boss.transform.position);
    ...
    void Movement()
    {
        if (!Boss.anim.GetCurrentAnimatorStateInfo(0).IsName("大招") && !Boss.anim.GetCurrentAnimatorStateInfo(0).IsName("吹风"))
        {
            if (Boss.dist > 4f && !isattack)
            {
                Move();
    ...
    void Move()
    {
        if (Boss.anim.GetCurrentAnimatorStateInfo(0).IsName("idle") || Boss.anim.GetCurrentAnimatorStateInfo(0).IsName("run"))
            Boss.transform.position = Vector2.MoveTowards(Boss.transform.position, Boss.player.position, 25 * Time.deltaTime);
    }
    ...
    void Attack()
    {
        ...
        cd_1 -= Time.deltaTime;
        if (cd_1 <= 0 && Boss.count_1 <= 1)
        {
            isattack = true;
            Boss.anim.Play("attack_2");
            cd_1 = 1.2f;
        }
        else if (cd_1 <= 0 && Boss.count_1 == 2)
        {
            isattack = true;
            Boss.anim.Play("attack_3");
            cd_1 = 1.2f;
        }
